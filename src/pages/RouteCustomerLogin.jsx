...
export default function RouteCustomerLogin() {
  ...
  const routeLogin =
    typeof routeAuth?.login === "function" ? routeAuth.login : null;

  const isRouteCustomerAuthed = Boolean(routeAuth?.isRouteCustomerAuthed);
  ...
  async function handleSubmit(e) {
    ...
    if (!routeLogin) {
      setError("Route customer auth is not ready. Check RouteCustomerAuthContext.");
      return;
    }
    ...
    try {
      await routeLogin(username, password);
      window.location.href = "/customer/dashboard";
    } catch (err) {
      setError(err?.message || "Route customer login failed.");
    } finally {
      setLoading(false);
    }
  }
    ...
    >
      ...
      >
        ...
        <div style={{ padding: 22 }}>
          ...
          >
            Sign in with your route portal username and password.
          </div>
